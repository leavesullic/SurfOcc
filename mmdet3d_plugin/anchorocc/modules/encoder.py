...
class AnchorOccEncoder(TransformerLayerSequence):
    ...
    def __init__(self, *args, dataset=None, grid_config=None, pc_range=None, num_points_in_pillar=4, return_intermediate=False, dataset_type='nuscenes',
                 **kwargs):

        super(AnchorOccEncoder, self).__init__(*args, **kwargs)
    ...
    @force_fp32(apply_to=('reference_points', 'pc_range', 'img_metas', 'cam_params'))
    def point_sampling(self, reference_points, pc_range,  img_metas, cam_params):
        ...
        if self.dataset == 'kitti':
            ...
            rots, trans, intrins, post_rots, post_trans, bda, lidar2img = cam_params
            ...
            reference_points = torch.inverse(bda).view(1, B, 1, 1, 4, 4).matmul(reference_points)
            ...
            reference_points_cam = post_rots.view(1, B, num_cam, 1, 3, 3).matmul(reference_points_cam.unsqueeze(-1)).squeeze(-1)
            reference_points_cam += post_trans.view(1, B, num_cam, 1, 3)
            ...
            reference_points = reference_points.view(
                D, B, 1, num_query, 3).repeat(1, 1, num_cam, 1, 1).unsqueeze(-1)  # D B NUM_CAM NUM_QUERY 3 1
        
            reference_points = torch.inverse(bda).view(1, B, 1, 1, 3, 3).matmul(reference_points).squeeze(-1)
            
            reference_points -= trans.view(1, B, num_cam, 1, 3)
            combine = rots.matmul(torch.inverse(intrins)).inverse()
            
            reference_points_cam = combine.view(1, B, num_cam, 1, 3, 3).repeat(D, 1, 1, 1, 1, 1).matmul(reference_points.unsqueeze(-1)).squeeze(-1)
    ...
    def forward(self,
        ...
        reference_points_cam, bev_mask = self.point_sampling(
            ref_3d, self.pc_range, kwargs['img_metas'], cam_params)
...
class AnchorOccLayer(MyCustomBaseTransformerLayer):
    ...
    def __init__(self,
                 ...
                 ffn_num_fcs=2,
                 **kwargs):
        super(AnchorOccLayer, self).__init__(
            attn_cfgs=attn_cfgs,
