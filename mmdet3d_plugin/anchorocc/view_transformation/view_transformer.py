...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor(self, rots, trans, intrins, post_rots, post_trans, bda):
        ...
        points = self.frustum.to(post_trans) - post_trans.view(B, N, 1, 1, 1, 3)
...
class DepthNet(nn.Module):
    ...
    def forward(self, x, mlp_input):
        # mlp_input = torch.rand(1, 33).to(x)
        mlp_input = self.bn(mlp_input.view(-1, mlp_input.shape[-1]))
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def __init__(self, loss_depth_weight=3.0, **kwargs):
        ...
        self.depth_net = DepthNet(self.in_channels, self.in_channels,
                                  self.out_channels, self.D, cam_channels=33)
    ...
    def get_mlp_input(self, rot, tran, intrin, post_rot, post_tran, bda=None):
        ...
        mlp_input = torch.cat([mlp_input, sensor2ego], dim=-1)   # (2, 1, 33)
...
@NECKS.register_module()
class LSSViewTransformerAnchorOcc(LSSViewTransformerBEVDepth):
    def __init__(self, bev_channels=None, bev_h=None, bev_w=None, anchor_channel=1, **kwargs):
        super(LSSViewTransformerAnchorOcc, self).__init__(**kwargs)
        self.bev_channels = bev_channels
        self.bev_h = bev_h
        self.bev_w = bev_w
        self.anchor_channel = anchor_channel
        
        # self.bev_embed = nn.Embedding(self.bev_h * self.bev_w, self.bev_channels)
        
        
    def anchor_transform(self, input, depth_onehot, anchor_feat):        
        if self.accelerate:
            self.pre_compute(input)
        return self.anchor_transform_core(input, depth_onehot, anchor_feat)
    
    def anchor_transform_core(self, input, depth_onehot, anchor_feat):
        B, N, C, H, W = input[0].shape

        # Lift-Splat
        if self.accelerate:
            feat = anchor_feat.view(B, N, self.anchor_channel, H, W)
            feat = feat.permute(0, 1, 3, 4, 2)
            depth_onehot = depth_onehot.view(B, N, self.D, H, W)
            bev_feat_shape = (depth_onehot.shape[0], int(self.grid_size[2]),
                              int(self.grid_size[1]), int(self.grid_size[0]),
                              feat.shape[-1])  # (B, Z, Y, X, C)
            bev_feat = bev_pool_v2(depth_onehot, feat, self.ranks_depth,
                                   self.ranks_feat, self.ranks_bev,
                                   bev_feat_shape, self.interval_starts,
                                   self.interval_lengths)
            ...
            coor = self.get_lidar_coor(*input[1:7])
            anchor_mask = self.voxel_pooling_v2(
                coor, depth_onehot.view(B, N, self.D, H, W),
                anchor_feat.view(B, N, self.anchor_channel, H, W))
        return anchor_mask
    
    def result_format(self, voxel_feat, anchor_mask):
        """
        anchor_mask : B 1 X Y Z
        """
        
        # get bev feat for parallel
        # bev_feat = torch.cat(voxel_feat.unbind(-1), 1)
        # bev_feat = voxel_feat.mean(-1)     
        
        # for voxel anchor
        # B anchorC X Y Z -> B X Y Z 
        voxel_anchor = anchor_mask.squeeze(1)
        
        # for BEV anchor
        # B anchorC X Y Z -> B X Y
        # bev_anchor = torch.cat(anchor_mask.unbind(dim=-1), 1)
        # bev_anchor = bev_anchor.permute(0, 2, 3, 1).mean(-1)
        #
        # return voxel_anchor, bev_anchor
        return voxel_anchor
    
    def forward(self, input):
        """
        returns:
        voxel feat torch.Size([2, 128, 64, 64, 8])
        bev feat torch.Size([2, 1024, 64, 64])
        voxel anchor torch.Size([2, 64, 64, 8])
        bev anchor torch.Size([2, 64, 64])
        ...
        # generate anchor and feature
        depth_onehot = depth.clone()
        depth_mask = (depth_onehot == depth_onehot.max(dim=1, keepdim=True)[0]).to(dtype=torch.int32)
        depth_onehot = torch.ceil(torch.mul(depth_mask, depth_onehot)).to(torch.int)
        
        
        anchor_feat = torch.ones_like(tran_feat).narrow(1, 0, 1)
        
        voxel_feat, depth = self.view_transform(input, depth, tran_feat)
        anchor_mask = self.anchor_transform(input, depth_onehot, anchor_feat)
        voxel_anchor = anchor_mask.squeeze(1)
        # bev_feat, voxel_anchor, bev_anchor = self.result_format(voxel_feat, anchor_mask)

        
        
        # bev_queries = self.bev_embed.weight.to(dtype)
        # bev_queries = bev_queries.unsqueeze(0).repeat(B, 1, 1)  # B xy dim
        
        # bev_feat = bev_feat.flatten(2).permute(0, 2, 1) + bev_queries
        # bev_feat = bev_feat.permute(0, 2, 1).reshape(B, self.bev_channels, self.bev_h, self.bev_w)

        # return voxel_feat, voxel_anchor, bev_anchor, depth
        return voxel_feat, voxel_anchor, depth
