...
def geo_scal_loss(pred, ssc_target, empty_idx):
...
def dice_loss(pred, ssc_target):
    ...
    for i in range(0, n_classes):
        ...
        if torch.sum(completion_target) > 0:
            ...
            union = torch.sum(p) + torch.sum(completion_target)
            dice = (2 * intersection + eps) / (union + eps)
            dice_loss_class = 1 - dice

            loss += dice_loss_class
